...
func (restManager *RestManager) Start() {
	restServer := cg.NewRestServer(mysqlManager)
            ...
            for i := snCount; i >= 1; i-- {   
                ...
                for _, deviceClass := range config.DeviceClasses {
                    ...
                    for _, entityClass := range device.DeviceClass.EntityClasses {
                        ...
                        // Assume dataManager is an object that has a method Push that accepts a DeviceData object
...
func (restManager *RestManager) isSearchDone(deviceData *DeviceData) bool {
    ...
    if deviceData.Device != nil {
        ...
        restManager.logSearchStatus(gatewayId)
...
// logSearchStatus handles the logging of search status.
func (restManager *RestManager) logSearchStatus(gatewayId string) {
    log.Printf("Search num is %d total is %d", restManager.searchNums[gatewayId], restManager.searchTotals[gatewayId])
}

// checkSearchCompletion checks if the search has been completed.
...
// updateCurrentSearchSN updates the current search SN.
